...
function Page() {
  const router = useRouter(); // ⬅️ Harus di dalam komponen!
  ...
  useEffect(() => {
    ...
    async function fetchModules() {
      ...
      if (selectedLab === "Lab RPL") apiUrl = "http://localhost:8080/api/lab/lab-rpl/1";
      else if (selectedLab === "Lab Sister") apiUrl = "http://localhost:8080/api/lab/lab-sister/4";
      else if (selectedLab === "Lab Jaringan") apiUrl = "http://localhost:8080/api/lab/lab-jaringan/2";
      else if (selectedLab === "Lab Web") apiUrl = "http://localhost:8080/api/lab/lab-mulmed/3";
      ...
      <aside className="w-64 bg-white h-screen shadow-lg flex flex-col justify-between p-4">
        ...
        <div className="relative mb-4">
          ...
          <nav className="mt-6">
            <ul className="space-y-4">
              {[ 
                { href: "/praktikan/jadwal", img: "/jadwal-praktikum.svg", label: "Jadwal Praktikum" },
                { href: "/praktikan/presensi", img: "/presensi.svg", label: "Presensi" },
                { href: "/praktikan/submission", img: "/submission.svg", label: "Submission" },
                { href: "/praktikan/penilaian", img: "/penilaian.svg", label: "Penilaian" }
              ].map((item, index) => (
        ...
        <div className="mt-10">
          ...
          <button
            ...
            className="flex items-center text-gray-600 px-4 py-2 hover:bg-gray-200 rounded-lg w-full"
          >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {modules.length > 0 ? (
              modules.map((module, index) => (
                ...
                >
                  ...
                  <Link
                    href={`/praktikum/prak-eldas/${module.id}`}
            ...
            <div className="col-span-1">
              <Link
                href="/add-practicum"
                className="w-full bg-green-500 text-white px-4 py-3 rounded-lg hover:bg-green-600 block text-center"
              >
                Add Practicum
              </Link>
            </div>
